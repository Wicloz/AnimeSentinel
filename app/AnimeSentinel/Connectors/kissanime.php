...
class kissanime extends BaseConnector
{
  ...
  protected static $page_main = 'http://kissanime.io';
  protected static $page_recent = 'http://kissanime.io';
  ...
  public static function findStreamPages($show) {
    ...
    foreach ($show->alts as $alt) {
      ...
      $page = Downloaders::downloadPage(Static::$page_main.'/Search/?s='.str_replace(' ', '+', $alt));

      // First check whether we already have an episode page
      if (str_contains($page, '| watch free anime series HD online')) {
        // Grab the partial link
        $linkPart = str_get_between($page, '<a Class="bigChar" href="//kissanime.io/', '">');
        ...
        $results = Helpers::scrape_page(str_get_between($page, '<div>Latest episode</div>', '<script'), '</div>', [
          'linkPart' => [true, '<a class="bigChar" href="//kissanime.io/', '">'],
          'title' => [false, '<a class="bigChar" href="//kissanime.io/{{linkPart}}">', '<'],
        ]);
  ...
  protected static function scrapeStreamPage($page) {
    $episodes = Helpers::scrape_page(str_get_between($page, '<div>Day Added</div>', '<script'), '</div>', [
      'link_episode' => [true, 'href="//kissanime.io/', '"'],
      'episode_num' => [false, 'online in high quality">', '</a>'],
      'uploadtime' => [false, '<div>', ''],
    ]);

    $title = trim(str_get_between($page, '<div class="barTitle full">', 'information'));
  ...
  protected static function completeEpisodeData($episode) {
    ...
    $episode['uploadtime'] = Carbon::createFromFormat('m/d/Y', trim($episode['uploadtime']))->setTime(0, 0, 0);
  ...
  protected static function scrapeRecentlyAired($page) {
    return Helpers::scrape_page(str_get_between($page, '<div class="owl-carousel owl5">', '<div class="clear2">'), '</a>', [
